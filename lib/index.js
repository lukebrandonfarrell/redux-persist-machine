 ...
 */
export const persistMiddleware = (save, load) => (next) => (action) => __awaiter(this, void 0, void 0, function* () {
    // Assign our save and load methods
    saveMethod = save;
    loadMethod = load;
    // Gets the our trigger actions
    const currentValueActionAndKeys = Object.entries(currentValue)
        .map((item) => { return { action: item[1].action, key: item[1].key }; });
    const targetActionAndKey = currentValueActionAndKeys.filter(item => (item.action === action.type))[0];
    ...
    if (!_isNil(targetActionAndKey)) {
        ...
        if (!_isNil(asyncStorageKey)) {
            ...
            let payload = yield load(asyncStorageKey);
            // Merge the payload received from our load function
            action.payload = Object.assign({}, payload, action.payload);
            // Update our current value target to isLoaded = true
            currentValue[asyncStorageKey] = Object.assign({}, _get(currentValue, asyncStorageKey, {}), { isLoaded: true });
        }
        else {
...
/**
 * Persist Machine - Method to persist state data
 *
 ...
 *
 */
export function persistMachine(structure, store, debug) {
    /*
    ...
    _map(structure, (object, name) => {
        ...
        currentValue[asyncStorageKey] = Object.assign({}, _get(currentValue, name, {}), { key: asyncStorageKey, action, isLoaded: false });
    });
    /**
    ...
    function handleChange() {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield _map(structure, (object, name) => __awaiter(this, void 0, void 0, function* () {
                ...
                currentValue[asyncStorageKey] = Object.assign({}, _get(currentValue, asyncStorageKey, {}), { key: asyncStorageKey, state: newState });
